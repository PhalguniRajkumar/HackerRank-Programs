//Question of this problem is below the program

#include <stdio.h>

int min(int width[],int start,int end);

main()
{

	int i,N,T,start,end;
	scanf("%d %d",&N,&T);//Here N is the length of highway and T is the number of test cases

	int width[N],result[T];//Width is an array consisting of width of each segment of the highway
	for(i=0;i<N;i++)
	{
		scanf("%d",&width[i]);
	}
    for(i=0;i<T;i++)
    {
    	scanf("%d %d",&start,&end);//start is index of the segment through which Calvin will enter and end is the index of the segment through which he will exit
    	result[i]=min(width,start,end);
    }
...
//This function will find min in the array width starting from index start to index end
int min(int width[],int start,int end)
{
	int i;
	int min=width[end];
